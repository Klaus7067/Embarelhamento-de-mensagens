...
function cifraCesar() {
  ...
  if (codificar.checked) {
    for (let i = 0; i < LowerCasedMessage.length; i++) {
      let currentLetter = LowerCasedMessage[i];
      ...
      if (!alphabet.includes(currentLetter)) {
        ...
        console.log(msg[i]);
        if (msg[i] === currentLetter) {
          saida += String.fromCharCode((currentLetter.charCodeAt(currentLetter) + chave - 97) % 26 + 97);
        }
        else {
          saida += String.fromCharCode((currentLetter.charCodeAt(currentLetter) + chave - 97) % 26 + 97).toUpperCase();
